...
class Achievements {
	public static function init()
	{
		...
		createAchievement('week7_nomiss',			{name: "Sock Joke", 			   description: "Beat Week 7 on Hard with no Misses."});
		createAchievement('weeksecret_nomiss',		{name: "Tightest Bars of All!",    description: "Beat Week ? with no Misses."});
		createAchievement('ur_bad',					{name: "What a Funkin' Disaster!", description: "Complete a Song with a rating lower than 20%."});
		createAchievement('ur_good',				{name: "Call it a P Rank!", 	   description: "Complete a Song with a rating of 100%."});
		...
		createAchievement('showoff',				{name: "Showing Off Much?",		   description: "Dodge 60 times in Mild and win.",});
		createAchievement('jr_gameclear',			{name: "Ready to rap Mama Peach",  description: "Clear all Weeks as Bowser Jr (Including Tutorial)", maxScore: 9, maxDecimals: 0});
		createAchievement('moni_gameclear',			{name: "Just Monika!",  		   description: "Clear all Weeks as Monika (Including Tutorial)", maxScore: 9, maxDecimals: 0});
		createAchievement('pizza_clear',			{name: "Peppino Approved", 		   description: "Beat the most secret song\n(Hint: \"Pizza Forever\" on DS Dialogue)"});
	...
	public static function unlock(name:String, autoStartPopup:Bool = true):String {
		...
		{
			FlxG.sound.play(Paths.sound('achievementGet'), 0.5); // Making the achievement get sound a seperate file so you can easily add your own
			_lastUnlock = time;
		}
