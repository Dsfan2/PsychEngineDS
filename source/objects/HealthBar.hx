...
class HealthBar extends FlxSpriteGroup
{
	...
	public var defaultHeight:Int = 19;
	public var heightDiff:Float;
	...
	public function new(x:Float, y:Float, image:String = 'healthBar-Default', valueFunction:Void->Float = null, boundX:Float = 0, boundY:Float = 1)
	{
		super(x, y);
		...
		bg = new FlxSprite().loadGraphic(Paths.image('bars/' + image));
		bg.antialiasing = ClientPrefs.data.antialiasing;
		barWidth = Std.int(bg.width - 6);
		barHeight = Std.int(bg.height - 6);

		if (image.startsWith('timeBar'))
		{
			heightDiff = bg.height - defaultHeight;
			bg.y -= heightDiff;
		}
