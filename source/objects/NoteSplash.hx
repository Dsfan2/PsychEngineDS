...
class NoteSplash extends FlxSprite
{
	...
	private var colArray:Array<String> = ['purple', 'blue', 'green', 'red', 'gold'];

	public static var defaultNoteSplash(default, never):String = 'Default';
	...
	public function new(x:Float = 0, y:Float = 0) {
		...
		else skin = 'noteSplashes/' + ClientPrefs.data.splashSkin;
	...
	public function setupNoteSplash(x:Float, y:Float, direction:Int = 0, ?note:Note = null, ?texture:String = null) {
		...
		var textureTxt:String = null;
		if (texture != null)
			note.noteSplashData.texture = texture;
		else
			note.noteSplashData.texture = PlayState.SONG.splashSkin;

		if(note != null && note.noteSplashData.texture != null) textureTxt = 'noteSplashes/' + note.noteSplashData.texture;
		else if(PlayState.SONG.splashSkin != null && PlayState.SONG.splashSkin.length > 0) textureTxt = 'noteSplashes/' + PlayState.SONG.splashSkin;
		else textureTxt = 'noteSplashes/' + ClientPrefs.data.splashSkin;
		
		var config:NoteSplashConfig = null;
		if(_textureLoaded != textureTxt)
			config = loadAnims(textureTxt);
		else
	...
	function loadAnims(skin:String, ?animName:String = null):NoteSplashConfig {
		...
		if(frames == null)
		{
			skin = 'noteSplashes/' + ClientPrefs.data.splashSkin;
			frames = Paths.getSparrowAtlas(skin);
			if(frames == null) //if you really need this, you really screwed something up
			{
				skin = 'noteSplashes/' + ClientPrefs.data.splashSkin;
				frames = Paths.getSparrowAtlas(skin);
			}
