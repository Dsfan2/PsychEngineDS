...
{
	var stageDirectory:String = 'shared';
	...
	override public function create()
	{
		...
		rating = new FlxSprite().loadGraphic(Paths.image('hudFolders/${ClientPrefs.data.comboHUDDir}UI/sick'));
		...
		{
			var numScore:FlxSprite = new FlxSprite(43 * daLoop).loadGraphic(Paths.image('hudFolders/${ClientPrefs.data.comboHUDDir}UI/num' + i));
		...
		var comboSpr:FlxSprite = new FlxSprite(43 * daLoop).loadGraphic(Paths.image('hudFolders/${ClientPrefs.data.comboHUDDir}UI/combo'));
		...
		beatText.setScale(0.6 * TitleState.alphabetScale, 0.6 * TitleState.alphabetScale);
		beatText.x += 260;
		beatText.alpha = 0;
		...
		timeBar = new HealthBar(0, timeTxt.y + (timeTxt.height / 3), 'healthBar-' + ClientPrefs.data.healthAndTimeBars, function() return barPercent, delayMin, delayMax);
