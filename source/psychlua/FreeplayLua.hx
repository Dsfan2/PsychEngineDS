...
import psychlua.LuaUtilsFreeplay.LuaTweenFP;
#if (SScript >= "3.0.0")
...
class FreeplayLua {
	...
	#if (SScript >= "3.0.0")
	public var hscript:HScriptFreeplay = null;
	#end
	...
	public function new(scriptName:String) {
		...
		set('curSong', FreeplayState.selectedItem);
		set('curDifficulty', FreeplayState.selectedDifficulty);
		set('songList', FreeplayState.instance.songList);
		...
		Lua_helper.add_callback(lua, "setVar", function(varName:String, value:Dynamic) {
			FreeplayState.instance.variables.set(varName, value);
			return value;
		});
		Lua_helper.add_callback(lua, "getVar", function(varName:String) {
			return FreeplayState.instance.variables.get(varName);
		});
		...
		Lua_helper.add_callback(lua, "addHScript", function(luaFile:String, ?ignoreAlreadyRunning:Bool = false) {
			...
			if(foundScript != null)
			{
				...
				FreeplayState.instance.initHScript(foundScript);
				return;
			}
		...
		Lua_helper.add_callback(lua, "loadGraphic", function(variable:String, image:String, ?gridX:Int = 0, ?gridY:Int = 0) {
			...
			var spr:FlxSprite = LuaUtilsFreeplay.getObjectDirectly(split[0]);
			var animated = gridX != 0 || gridY != 0;

			if(split.length > 1) {
				spr = LuaUtilsFreeplay.getVarInArray(LuaUtilsFreeplay.getPropertyLoop(split), split[split.length-1]);
			}
		...
		Lua_helper.add_callback(lua, "loadFrames", function(variable:String, image:String, spriteType:String = "sparrow") {
			...
			if(spr != null && image != null && image.length > 0)
			{
				LuaUtilsFreeplay.loadFrames(spr, image, spriteType);
			}
		});

		//ebic stuff for epic coders like me B)  *image of obama giving himself a medal*
		Lua_helper.add_callback(lua, "getObjectOrder", function(obj:String) {
			var split:Array<String> = obj.split('.');
			var leObj:FlxBasic = LuaUtilsFreeplay.getObjectDirectly(split[0]);
			if(split.length > 1) {
			...
			if(leObj != null)
			{
				return LuaUtilsFreeplay.getTargetInstance().members.indexOf(leObj);
			}
		...
		Lua_helper.add_callback(lua, "setObjectOrder", function(obj:String, position:Int) {
			...
			if(leObj != null) {
				LuaUtilsFreeplay.getTargetInstance().remove(leObj, true);
				LuaUtilsFreeplay.getTargetInstance().insert(position, leObj);
				return;
			}
		...
		Lua_helper.add_callback(lua, "startTween", function(tag:String, vars:String, values:Any = null, duration:Float, options:Any = null) {
			var pipis:Dynamic = LuaUtilsFreeplay.tweenPrepare(tag, vars);
			if(pipis != null) {
				if(values != null) {
					var myOptions:LuaTweenFP = LuaUtilsFreeplay.getLuaTween(options);
		...
		Lua_helper.add_callback(lua, "doTweenColor", function(tag:String, vars:String, targetColor:String, duration:Float, ease:String) {
			...
			if(pipis != null) {
				...
				game.modchartTweens.set(tag, FlxTween.color(pipis, duration, curColor, CoolUtil.colorFromString(targetColor), {ease: LuaUtilsFreeplay.getTweenEaseByString(ease),
					onComplete: function(twn:FlxTween) {
		...
		Lua_helper.add_callback(lua, "cancelTween", function(tag:String) {
			LuaUtilsFreeplay.cancelTween(tag);
		});

		Lua_helper.add_callback(lua, "runTimer", function(tag:String, time:Float = 1, loops:Int = 1) {
			LuaUtilsFreeplay.cancelTimer(tag);
		...
		Lua_helper.add_callback(lua, "cameraShake", function(camera:String, intensity:Float, duration:Float) {
			LuaUtilsFreeplay.cameraFromString(camera).shake(intensity, duration);
		});

		Lua_helper.add_callback(lua, "cameraFlash", function(camera:String, color:String, duration:Float,forced:Bool) {
			LuaUtilsFreeplay.cameraFromString(camera).flash(CoolUtil.colorFromString(color), duration, null,forced);
		});
		Lua_helper.add_callback(lua, "cameraFade", function(camera:String, color:String, duration:Float,forced:Bool) {
			LuaUtilsFreeplay.cameraFromString(camera).fade(CoolUtil.colorFromString(color), duration, false,null,forced);
		});
		Lua_helper.add_callback(lua, "getMouseX", function(camera:String) {
			var cam:FlxCamera = LuaUtilsFreeplay.cameraFromString(camera);
		...
		Lua_helper.add_callback(lua, "getMidpointX", function(variable:String) {
			var split:Array<String> = variable.split('.');
			var obj:FlxSprite = LuaUtilsFreeplay.getObjectDirectly(split[0]);
			if(split.length > 1) {
				obj = LuaUtilsFreeplay.getVarInArray(LuaUtilsFreeplay.getPropertyLoop(split), split[split.length-1]);
			}
		...
		Lua_helper.add_callback(lua, "makeLuaSprite", function(tag:String, ?image:String = null, ?x:Float = 0, ?y:Float = 0) {
			tag = tag.replace('.', '');
			LuaUtilsFreeplay.resetSpriteTag(tag);
		...
		Lua_helper.add_callback(lua, "makeAnimatedLuaSprite", function(tag:String, ?image:String = null, ?x:Float = 0, ?y:Float = 0, ?spriteType:String = "sparrow") {
			...
			LuaUtilsFreeplay.loadFrames(leSprite, image, spriteType);
			game.modchartSprites.set(tag, leSprite);
		});

		Lua_helper.add_callback(lua, "makeGraphic", function(obj:String, width:Int = 256, height:Int = 256, color:String = 'FFFFFF') {
			var spr:FlxSprite = LuaUtilsFreeplay.getObjectDirectly(obj, false);
			if(spr != null) spr.makeGraphic(width, height, CoolUtil.colorFromString(color));
		});
		Lua_helper.add_callback(lua, "addAnimationByPrefix", function(obj:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			var obj:Dynamic = LuaUtilsFreeplay.getObjectDirectly(obj, false);
		...
		Lua_helper.add_callback(lua, "addAnimationByIndices", function(obj:String, name:String, prefix:String, indices:String, framerate:Int = 24, loop:Bool = false) {
			return LuaUtilsFreeplay.addAnimByIndices(obj, name, prefix, indices, framerate, loop);
		});
		...
		Lua_helper.add_callback(lua, "setScrollFactor", function(obj:String, scrollX:Float, scrollY:Float) {
			...
			var object:FlxObject = Reflect.getProperty(LuaUtilsFreeplay.getTargetInstance(), obj);
			if(object != null) {
		...
		Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, front:Bool = false) {
			if(game.modchartSprites.exists(tag)) {
				...
				if(front)
					LuaUtilsFreeplay.getTargetInstance().add(trash);
				else
				{
		...
		Lua_helper.add_callback(lua, "setGraphicSize", function(obj:String, x:Int, y:Int = 0, updateHitbox:Bool = true) {
			...
			var split:Array<String> = obj.split('.');
			var poop:FlxSprite = LuaUtilsFreeplay.getObjectDirectly(split[0]);
			if(split.length > 1) {
				poop = LuaUtilsFreeplay.getVarInArray(LuaUtilsFreeplay.getPropertyLoop(split), split[split.length-1]);
			}
		...
		Lua_helper.add_callback(lua, "updateHitbox", function(obj:String) {
			...
			var poop:FlxSprite = Reflect.getProperty(LuaUtilsFreeplay.getTargetInstance(), obj);
			if(poop != null) {
				poop.updateHitbox();
				return;
			}
		...
		Lua_helper.add_callback(lua, "updateHitboxFromGroup", function(group:String, index:Int) {
			if(Std.isOfType(Reflect.getProperty(LuaUtilsFreeplay.getTargetInstance(), group), FlxTypedGroup)) {
				Reflect.getProperty(LuaUtilsFreeplay.getTargetInstance(), group).members[index].updateHitbox();
				return;
			}
			Reflect.getProperty(LuaUtilsFreeplay.getTargetInstance(), group)[index].updateHitbox();
		});
		...
		Lua_helper.add_callback(lua, "removeLuaSprite", function(tag:String, destroy:Bool = true) {
			...
			LuaUtilsFreeplay.getTargetInstance().remove(pee, true);
			if(destroy) {
				pee.destroy();
		...
		Lua_helper.add_callback(lua, "makeLuaBackdrop", function(tag:String, image:String) {
			tag = tag.replace('.', '');
			LuaUtilsFreeplay.resetBackdropTag(tag);
		...
		Lua_helper.add_callback(lua, "removeLuaBackdrop", function(tag:String, destroy:Bool = true) {
			...
			LuaUtilsFreeplay.getTargetInstance().remove(sophist, true);
			if(destroy) {
				sophist.destroy();
		...
		Lua_helper.add_callback(lua, "setObjectVelocity", function(obj:String, velX:Int = 0, velY:Int = 0) {
			...
			var pipebomb:FlxSprite = LuaUtilsFreeplay.getObjectDirectly(killMe[0]);
			if(killMe.length > 1) {
				pipebomb = LuaUtilsFreeplay.getVarInArray(LuaUtilsFreeplay.getPropertyLoop(killMe), killMe[killMe.length-1]);
			}
		...
		Lua_helper.add_callback(lua, "setObjectCamera", function(obj:String, camera:String = '') {
			...
			if(real!=null){
				real.cameras = [LuaUtilsFreeplay.cameraFromString(camera)];
				return true;
			}

			var split:Array<String> = obj.split('.');
			var object:FlxSprite = LuaUtilsFreeplay.getObjectDirectly(split[0]);
			if(split.length > 1) {
				object = LuaUtilsFreeplay.getVarInArray(LuaUtilsFreeplay.getPropertyLoop(split), split[split.length-1]);
			}

			if(object != null) {
				object.cameras = [LuaUtilsFreeplay.cameraFromString(camera)];
				return true;
			}
		...
		Lua_helper.add_callback(lua, "setBlendMode", function(obj:String, blend:String = '') {
			var real = game.getLuaObject(obj);
			if(real != null) {
				real.blend = LuaUtilsFreeplay.blendModeFromString(blend);
				return true;
			}
			...
			if(spr != null) {
				spr.blend = LuaUtilsFreeplay.blendModeFromString(blend);
				return true;
			}
		...
		Lua_helper.add_callback(lua, "screenCenter", function(obj:String, pos:String = 'xy') {
			...
			if(spr==null){
				var split:Array<String> = obj.split('.');
				spr = LuaUtilsFreeplay.getObjectDirectly(split[0]);
				if(split.length > 1) {
					spr = LuaUtilsFreeplay.getVarInArray(LuaUtilsFreeplay.getPropertyLoop(split), split[split.length-1]);
				}
			}
		...
		Lua_helper.add_callback(lua, "objectsOverlap", function(obj1:String, obj2:String) {
			...
			for (i in 0...namesArray.length)
			{
				...
				if(real!=null) {
					...
					objectsArray.push(Reflect.getProperty(LuaUtilsFreeplay.getTargetInstance(), namesArray[i]));
				}
			}
		...
		Lua_helper.add_callback(lua, "debugPrint", function(text:Dynamic = '', color:String = 'WHITE') FreeplayState.instance.addTextToDebug(text, CoolUtil.colorFromString(color)));
		...
		#if (SScript >= "3.0.0") HScriptFreeplay.implement(this); #end
		ReflectionFunctions.implementFreeplay(this);
		TextFunctions.implementFreeplay(this);
		ExtraFunctions.implementFreeplay(this);
		CustomSubstateFreeplay.implement(this);
		ShaderFunctions.implementFreeplay(this);
		DeprecatedFunctions.implementFreeplay(this);
	...
	public static var lastCalledScript:FreeplayLua = null;
		...
		try {
			...
			if (type != Lua.LUA_TFUNCTION) {
				if (type > Lua.LUA_TNIL)
					luaTrace("ERROR (" + func + "): attempt to call a " + LuaUtilsFreeplay.typeToString(type) + " value", false, false, FlxColor.RED);
	...
	public function stop() {
		#if LUA_ALLOWED
		FreeplayState.instance.luaArray.remove(this);
		closed = true;
	...
	{
		#if LUA_ALLOWED
		var target:Dynamic = LuaUtilsFreeplay.tweenPrepare(tag, vars);
		if(target != null) {
			FreeplayState.instance.modchartTweens.set(tag, FlxTween.tween(target, tweenValue, duration, {ease: LuaUtilsFreeplay.getTweenEaseByString(ease),
				onComplete: function(twn:FlxTween) {
					FreeplayState.instance.modchartTweens.remove(tag);
					FreeplayState.instance.callOnLuas('onTweenCompleted', [tag, vars]);
				}
			}));
		} else {
