...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		// Dodging Mechanic
		set('canDodge', PlayState.bfCanDodge);
		...
		// Noteskin/Splash
		set('noteSkin', PlayState.SONG.arrowSkin);
		set('splashSkin', PlayState.SONG.splashSkin);
		set('splashAlpha', ClientPrefs.data.splashAlpha);

		// Extra Psych DS Exclusive stuff
		set('playerChar', ClientPrefs.data.playerChar);
		...
		set('barSkin', PlayState.SONG.barSkin);
		...
		//garbo stuff for epic coders like me B)  *image of obama giving himself a medal*
		// You're not an epic coder ShadowMario, don't get ahead of yourself...
		// Though technically, neither am I an epic coder...
		...
		Lua_helper.add_callback(lua, "startTween", function(tag:String, vars:String, values:Any = null, duration:Float, options:Any = null) {
			var pipis:Dynamic = LuaUtils.tweenPrepare(tag, vars);
			if(pipis != null) {
				if(values != null) {
		...
		Lua_helper.add_callback(lua, "doTweenColor", function(tag:String, vars:String, targetColor:String, duration:Float, ease:String) {
			...
			if(pipis != null) {
				...
				game.modchartTweens.set(tag, FlxTween.color(pipis, duration, curColor, CoolUtil.colorFromString(targetColor), {ease: LuaUtils.getTweenEaseByString(ease),
					onComplete: function(twn:FlxTween) {
		...
		Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, front:Bool = false) {
			if(game.modchartSprites.exists(tag)) {
				var crud:ModchartSprite = game.modchartSprites.get(tag);
				if(front)
					LuaUtils.getTargetInstance().add(crud);
				else
				{
					if(!game.isDead && !game.isOnResults)
						game.insert(game.members.indexOf(LuaUtils.getLowestCharacterGroup()), crud);
					else
					{
						if (!game.isOnResults)
							GameOverSubstate.instance.insert(GameOverSubstate.instance.members.indexOf(GameOverSubstate.instance.boyfriend), crud);
						else
							ResultsScreen.instance.insert(ResultsScreen.instance.members.indexOf(ResultsScreen.instance.charSprite), crud);
					}
				}
			}
		});
		Lua_helper.add_callback(lua, "setGraphicSize", function(obj:String, x:Int, y:Int = 0, updateHitbox:Bool = true) {
			if(game.getLuaObject(obj)!=null) {
				var thing:FlxSprite = game.getLuaObject(obj);
				thing.setGraphicSize(x, y);
				if(updateHitbox) thing.updateHitbox();
				return;
			}
		...
		Lua_helper.add_callback(lua, "scaleObject", function(obj:String, x:Float, y:Float, updateHitbox:Bool = true) {
			if(game.getLuaObject(obj)!=null) {
				var yomama:FlxSprite = game.getLuaObject(obj);
				yomama.scale.set(x, y);
				if(updateHitbox) yomama.updateHitbox();
				return;
			}
		...
		Lua_helper.add_callback(lua, "updateHitbox", function(obj:String) {
			if(game.getLuaObject(obj)!=null) {
				var pizza:FlxSprite = game.getLuaObject(obj);
				pizza.updateHitbox();
				return;
			}
				...
				else
				{
					...
					else
					{
						if (!game.isOnResults)
							...
							ResultsScreen.instance.insert(ResultsScreen.instance.members.indexOf(ResultsScreen.instance.charSprite), trash);
					}
				}
			}
		});
		...
		Lua_helper.add_callback(lua, "setObjectVelocity", function(obj:String, velX:Int = 0, velY:Int = 0) {
			...
			var pipebomb:FlxSprite = LuaUtils.getObjectDirectly(killMe[0]);
			if(killMe.length > 1) {
				pipebomb = LuaUtils.getVarInArray(LuaUtils.getPropertyLoop(killMe), killMe[killMe.length-1]);
			}
		...
		Lua_helper.add_callback(lua, "addLuaVideoSprite", function(tag:String, front:Bool = false) {
			if(game.modchartVideos.exists(tag)) {
				var trash:ModchartVideo = game.modchartVideos.get(tag);
				if(front)
					LuaUtils.getTargetInstance().add(trash);
				else
				{
		...
		Lua_helper.add_callback(lua, "setVideoSpriteTime", function(tag:String, time:Int = 0) {
			if(!game.modchartVideos.exists(tag)) {
				return;
			}
			var sophist:ModchartVideo = game.modchartVideos.get(tag);
		...
		Lua_helper.add_callback(lua, "startDialogue", function(dialogueFile:String, music:String = null) {
			...
			#end
			{
				var trash:DialogueFile = DialogueBoxPsych.parseDialogue(path);
				if(trash.dialogue.length > 0) {
					game.startDialogue(trash, music);
		...
		Lua_helper.add_callback(lua, "startDSDialogue", function(dialogueFile:String, music:String = null) {
			var path:String;
			#if MODS_ALLOWED
			path = Paths.modsJson(Paths.formatToSongPath(PlayState.SONG.song) + '/' + dialogueFile);
			if(!FileSystem.exists(path))
			...
			luaTrace('startDSDialogue: Trying to load dialogue: ' + path);
			...
			#end
			{
				var trash:DialogueFileDS = DialogueBoxDS.parseDialogue(path);
				if(trash.dialogue.length > 0) {
					game.startDSDialogue(trash, music);
					luaTrace('startDSDialogue: Successfully loaded dialogue', false, false, FlxColor.GREEN);
					return true;
				} else {
					luaTrace('startDSDialogue: Your dialogue file is badly formatted!', false, false, FlxColor.RED);
				}
			} else {
				luaTrace('startDSDialogue: Dialogue file not found', false, false, FlxColor.RED);
				if(game.endingSong) {
					if (PlayState.isStoryMode)
						game.endSong();
					else
		...
		Lua_helper.add_callback(lua, "startVideo", function(videoFile:String) {
			...
			if(FileSystem.exists(Paths.video(videoFile))) {
				game.prepareVideo(videoFile);
				luaTrace('startVideo: Successfully loaded video', false, false, FlxColor.GREEN);
				return true;
			} else {
