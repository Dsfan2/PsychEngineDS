...
class LuaUtils
{
	...
	public static function resetVideoTag(tag:String) {
		...
		var target:ModchartVideo = PlayState.instance.modchartVideos.get(tag);
		target.kill();
		PlayState.instance.remove(target, true);
		target.destroy();
	...
	public static function tweenPrepare(tag:String, vars:String) {
		...
		var propThing:Dynamic = LuaUtils.getObjectDirectly(variables[0]);
		if(variables.length > 1) propThing = LuaUtils.getVarInArray(LuaUtils.getPropertyLoop(variables), variables[variables.length-1]);
		return propThing;
	}
	...
	public static function cameraFromString(cam:String):FlxCamera {
		switch(cam.toLowerCase()) {
			...
			case 'camdialogue' | 'dialogue': return PlayState.instance.camDialogue;
		}
