...
class LuaUtilsMainMenu
{
	...
	public static function setVarInArray(instance:Dynamic, variable:String, value:Dynamic, allowMaps:Bool = false):Any
	{
		...
		if(splitProps.length > 1)
		{
			...
			if(MainMenuState.instance.variables.exists(splitProps[0]))
			{
				var retVal:Dynamic = MainMenuState.instance.variables.get(splitProps[0]);
				if(retVal != null)
					target = retVal;
			}
	...
	public static function getObjectDirectly(objectName:String, ?checkForTextsToo:Bool = true, ?allowMaps:Bool = false):Dynamic
	{
		switch(objectName)
		{
			...
			default:
				var obj:Dynamic = MainMenuState.instance.getLuaObject(objectName, checkForTextsToo);
	...
	{
		return #if LUA_ALLOWED MainMenuState.instance.modchartTexts.exists(name) ? MainMenuState.instance.modchartTexts.get(name) : #end Reflect.getProperty(MainMenuState.instance, name);
	}
	...
	public static function resetTextTag(tag:String) {
		...
		var target:FlxText = MainMenuState.instance.modchartTexts.get(tag);
		target.kill();
		MainMenuState.instance.remove(target, true);
		target.destroy();
	...
	public static function resetSpriteTag(tag:String) {
		...
		var target:ModchartSprite = MainMenuState.instance.modchartSprites.get(tag);
		target.kill();
		MainMenuState.instance.remove(target, true);
		target.destroy();
	...
	public static function resetBackdropTag(tag:String) {
		#if LUA_ALLOWED
		if(!MainMenuState.instance.modchartBackdrops.exists(tag)) {
			return;
		}

		var target:ModchartBackdrop = MainMenuState.instance.modchartBackdrops.get(tag);
		target.kill();
		MainMenuState.instance.remove(target, true);
		target.destroy();
		MainMenuState.instance.modchartBackdrops.remove(tag);
		#end
	}
	...
	public static function tweenPrepare(tag:String, vars:String) {
		...
		var propThing:Dynamic = LuaUtilsMainMenu.getObjectDirectly(variables[0]);
		if(variables.length > 1) propThing = LuaUtilsMainMenu.getVarInArray(LuaUtilsMainMenu.getPropertyLoop(variables), variables[variables.length-1]);
		return propThing;
	}
	...
	public static function cameraFromString(cam:String):FlxCamera {
		switch(cam.toLowerCase()) {
			case 'camachievement' | 'achievement' | 'hud': return MainMenuState.instance.camAchievement;
		}
		return MainMenuState.instance.camMain;
	}
}