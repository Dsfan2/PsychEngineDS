...
class AchievementsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		progressBar = new HealthBar(0, descText.y + 52, 'healthBar-Default');
	...
	override function update(elapsed:Float)
	{
		...
		if(controls.ACCEPT)
		{
			if(onYes)
			{
				...
				if (option.name == 'jr_gameclear'){
					PlayState.jrWeeksCleared = null;
					FlxG.save.data.jrWeeksCleared = null;
				}
				if (option.name == 'moni_gameclear'){
					PlayState.moniWeeksCleared = null;
					FlxG.save.data.moniWeeksCleared = null;
				}
				Achievements.save();
				FlxG.save.flush();
