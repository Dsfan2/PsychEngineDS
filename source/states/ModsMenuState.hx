...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if(modsList.all.length < 1)
		{
			...
			if(FlxG.random.bool(0.1)) noModsTxt.text = 'IMAGINE BEING IN LAST PLACE\nWITH ZERO STARS?!'; //Toxic Carried Nick 3
		...
		psychdsOnlyText = new FlxText(bgDescription.x + 15, bgDescription.y + bgDescription.height - myHeight - 50, bgDescription.width - 30, "* This Mod will ONLY work with Psych Engine DS.", 16);
		psychdsOnlyText.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT);
		add(psychdsOnlyText);
	...
	function updateModDisplayData()
	{
		...
		psychdsOnlyText.visible = curMod.psychdsOnly;
...
class ModItem extends FlxSpriteGroup
{
	...
	public var psychdsOnly:Bool = false;
	...
	public function new(folder:String)
	{
		...
		if(pack != null)
		{
			...
			this.psychdsOnly = (pack.exclusive == true);
		}
		text.text = this.name;
