...
class CharacterEditorState extends MusicBeatState
{
	...
	function reloadBGs() {
		...
		if(onPixelBG) {
			...
			var bgSky:BGSprite = new BGSprite('stages/weeb/weebSky', OFFSET_X - (playerXDifference / 2) - 300, 0 - playerYDifference, 0.1, 0.1);
			bgLayer.add(bgSky);
			bgSky.antialiasing = false;

			var repositionCrud = -200 + OFFSET_X - playerXDifference;

			var bgSchool:BGSprite = new BGSprite('stages/weeb/weebSchool', repositionCrud, -playerYDifference + 6, 0.6, 0.90);
			bgLayer.add(bgSchool);
			bgSchool.antialiasing = false;

			var bgStreet:BGSprite = new BGSprite('stages/weeb/weebStreet', repositionCrud, -playerYDifference, 0.95, 0.95);
			bgLayer.add(bgStreet);
			bgStreet.antialiasing = false;

			var widJunk = Std.int(bgSky.width * 6);
			var bgTrees:FlxSprite = new FlxSprite(repositionCrud - 380, -800 - playerYDifference);
	...
	function addSettingsUI() {
		...
		check_player.checked = (daAnim.startsWith('bf') || daAnim.startsWith('jr') || daAnim.startsWith('monika')) && !daAnim.endsWith('opponent');
		...
		var templateCharacter:FlxButton = new FlxButton(140, 50, "Load Template", function()
		{
			...
			for (character in characters)
			{
				...
				character.player2Char = parsedJson.player2;
				character.player3Char = parsedJson.player3;
	...
	var p2CharInputText:FlxUIInputText;
	var p3CharInputText:FlxUIInputText;
	...
	function addCharacterUI() {
		...
		p2CharInputText = new FlxUIInputText(15, healthColorStepperR.y + 35, 75, char.player2Char, 8);
		p3CharInputText = new FlxUIInputText(p2CharInputText.x + 85, healthColorStepperR.y + 35, 75, char.player3Char, 8);
		...
		tab_group.add(new FlxText(15, p2CharInputText.y - 18, 0, 'P2 Character:'));
		tab_group.add(new FlxText(p3CharInputText.x, p3CharInputText.y - 18, 0, 'P3 Character:'));
		...
		tab_group.add(p2CharInputText);
		tab_group.add(p3CharInputText);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			else if(sender == p2CharInputText) {
				char.player2Char = p2CharInputText.text;
			}
			else if(sender == p3CharInputText) {
				char.player3Char = p3CharInputText.text;
			}
	...
	function genBoyOffsets():Void
	{
		...
		for (anim in char.animationsArray)
		{
			var text:FlxText = dumbTexts.recycle(FlxText);
			text.x = 10;
			text.y = 20 + (20 * daLoop);
			text.fieldWidth = 400;
	...
	function reloadCharacterOptions() {
		if(UI_characterbox != null) {
			...
			p2CharInputText.text = char.player2Char;
			p3CharInputText.text = char.player3Char;
			reloadAnimationDropDown();
			updatePresence();
		}
	}
	...
	override function update(elapsed:Float)
	{
		...
		var inputTexts:Array<FlxUIInputText> = [animationInputText, imageInputText, healthIconInputText, animationNameInputText, animationIndicesInputText, p2CharInputText, p3CharInputText];
	...
	function saveCharacter() {
		var json = {
			...
			"player2": char.player2Char,
			"player3": char.player3Char
		};
