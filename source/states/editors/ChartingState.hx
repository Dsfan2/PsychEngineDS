...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Dadbattle Spotlight', "Normally Used in Dad Battle, but now it can be used everywhere else!\nValue 1: 0/1 = ON/OFF,\n2 = Target Dad\n3 = Target BF\nValue 2: 0/1 = Smoke Enabled/Disabled"],
		['Hey!', "Plays the \"Hey!\" animation from Bopeebo,\nValue 1: BF = Only Boyfriend, GF = Only Girlfriend,\nSomething else = Both.\nValue 2: Custom animation duration,\nleave it blank for 0.6s"],
		['Set GF Speed', "Sets GF head bopping speed,\nValue 1: 1 = Normal speed,\n2 = 1/2 speed, 4 = 1/4 speed etc.\nUsed on Fresh during the beatbox parts.\n\nWarning: Value must be integer!"],
		['Philly Glow', "Now available in ALL stages!\nValue 1: 0/1/2 = OFF/ON/Reset Gradient\n \nTake that ShadowMario!"],
		['Kill Henchmen', "For Mom's songs, don't use this please, i love them :("],
		['Add Camera Zoom', "Used on MILD on that one \"hard\" part\nValue 1: Camera zoom add (Default: 0.015)\nValue 2: UI zoom add (Default: 0.03)\nLeave the values blank if you want to use Default."],
		...
		['Warning', "Shows Warning Sign.\nValues are meaningless"],
		['Dodge Event', "Value 1: Amount of time before impact\n(0.17 is default value, must be less than 1)\nValue 2: How much health you lose by not dodging\n(2 is default value, must be less than or equal to 2)"],
		['Pole Zoom', "Makes the pole that normally kills the henchmen\ntry to kill you instead.\n(Used with \"Dodge Event\")\n(Only used in the \"Limo\" Stage)"]
	];
	...
	public static var CHANGEABLE_GRID_SIZE:Int = 40;
	...
	**/
	...
	// lol I kinda reused some of the lilBuddies code from Rozebud
	...
	override function create()
	{
		...
		lilStage = new FlxSprite(10, 625).loadGraphic(Paths.image("chartingStateThings/plat"));
		lilStage.scrollFactor.set();
		lilStage.antialiasing = false;
		lilStage.setGraphicSize(Std.int(lilStage.width * 0.6));
		lilStage.updateHitbox();
		add(lilStage);

		lilBf = new FlxSprite(129, 490);
		lilBf.frames = Paths.getSparrowAtlas('chartingStateThings/lilBf');
		lilBf.animation.addByPrefix('idle', "Idle", 12, true);
		lilBf.animation.addByPrefix('0', "Left", 12, false);
		lilBf.animation.addByPrefix('1', "Down", 12, false);
		lilBf.animation.addByPrefix('2', "Up", 12, false);
		lilBf.animation.addByPrefix('3', "Right", 12, false);
		lilBf.animation.play('idle');
		lilBf.antialiasing = false;
		lilBf.setGraphicSize(Std.int(lilBf.width * 0.7));
		lilBf.updateHitbox();
		lilBf.animation.finishCallback = function(name:String){
			lilBf.animation.play(name, false, false, lilBf.animation.getByName(name).numFrames - 2);
		}
		lilBf.scrollFactor.set();
		add(lilBf);

		lilOpp = new FlxSprite(-20, 490);
		lilOpp.frames = Paths.getSparrowAtlas('chartingStateThings/lilOpponent');
		lilOpp.animation.addByPrefix('idle', "Idle", 12, true);
		lilOpp.animation.addByPrefix('0', "Left", 12, false);
		lilOpp.animation.addByPrefix('1', "Down", 12, false);
		lilOpp.animation.addByPrefix('2', "Up", 12, false);
		lilOpp.animation.addByPrefix('3', "Right", 12, false);
		lilOpp.animation.play('idle');
		lilOpp.antialiasing = false;
		lilOpp.setGraphicSize(Std.int(lilOpp.width * 0.7));
		lilOpp.updateHitbox();
		lilOpp.animation.finishCallback = function(name:String){
			lilOpp.animation.play(name, false, false, lilOpp.animation.getByName(name).numFrames - 2);
		}
		...
		// The box is bigger to account for some of the stuff I added...
		...
		\nA/D or Left/Right - Go to the previous/next section
		\nJ/L - Change Snap
	...
	function addSongUI():Void
	{
		...
		{
			PlayState.SONG = Song.parseJSONcrud(FlxG.save.data.autosave);
	...
	function addSectionUI():Void
	{
		...
		var duetButton:FlxButton = new FlxButton(10, copyLastButton.y + 45, "Duet Notes", function()
		{
			...
			for (note in _song.notes[curSec].sectionNotes)
			{
				...
				var copiedNote:Array<Dynamic> = [note[0], yoob, note[2], note[3]];
		...
		var mirrorButton:FlxButton = new FlxButton(duetButton.x + 100, duetButton.y, "Mirror Notes", function()
		{
			...
			for (note in _song.notes[curSec].sectionNotes)
			{
				...
				note[1] = goob;
				var copiedNote:Array<Dynamic> = [note[0], goob, note[2], note[3]];
			}
	...
	function addChartingUI() {
		...
		lilBuddiesBox = new FlxUICheckBox(check_mute_inst.x + 130, 200, null, null, "Lil' Buddies", 100);
	...
	var resultsBGInputText:FlxUIInputText;
	var resultsSpriteInputText:FlxUIInputText;
	var resultsSoundInputText:FlxUIInputText;
	var resultsMusicInputText:FlxUIInputText;
	var noteSkinInputText:FlxUIInputText;
	var noteSplashesInputText:FlxUIInputText;
	var barSkinInputText:FlxUIInputText;
	function addDataUI()
	{
		...
		//
		resultsBGInputText = new FlxUIInputText(200, 25, 150, _song.resultBG != null ? _song.resultBG : '', 8);
		blockPressWhileTypingOn.push(resultsBGInputText);
		
		resultsSpriteInputText = new FlxUIInputText(200, gameOverCharacterInputText.y + 35, 150, _song.resultSprite != null ? _song.resultSprite : '', 8);
		blockPressWhileTypingOn.push(resultsSpriteInputText);
		
		resultsSoundInputText = new FlxUIInputText(200, gameOverSoundInputText.y + 35, 150, _song.resultSound != null ? _song.resultSound : '', 8);
		blockPressWhileTypingOn.push(resultsSoundInputText);
		
		resultsMusicInputText = new FlxUIInputText(200, gameOverLoopInputText.y + 35, 150, _song.resultMusic != null ? _song.resultMusic : '', 8);
		blockPressWhileTypingOn.push(resultsMusicInputText);
		//
		...
		var dsFilterOptions:Array<String> = ['Default', 'Off', 'On'];

		var dsFilterDropDown = new FlxUIDropDownMenu(check_disableNoteRGB.x, check_disableNoteRGB.y + 40, FlxUIDropDownMenu.makeStrIdLabelArray(dsFilterOptions, true), function(option:String)
		{
			_song.forceDSFilter = dsFilterOptions[Std.parseInt(option)];
		});
		dsFilterDropDown.selectedLabel = _song.forceDSFilter;
		blockPressWhileScrolling.push(dsFilterDropDown);
		...
		barSkinInputText = new FlxUIInputText(200, noteSkinInputText.y, 150, _song.barSkin != null ? _song.barSkin : '', 8);
		blockPressWhileTypingOn.push(barSkinInputText);
		...
		tab_group_data.add(resultsBGInputText);
		tab_group_data.add(resultsSpriteInputText);
		tab_group_data.add(resultsSoundInputText);
		tab_group_data.add(resultsMusicInputText);
		...
		tab_group_data.add(barSkinInputText);
		...
		tab_group_data.add(new FlxText(resultsBGInputText.x, resultsBGInputText.y - 15, 0, 'Results Screen Background (images/):'));
		tab_group_data.add(new FlxText(resultsSpriteInputText.x, resultsSpriteInputText.y - 15, 0, 'Results Screen Sprite (images/results/):'));
		tab_group_data.add(new FlxText(resultsSoundInputText.x, resultsSoundInputText.y - 15, 0, 'Results Screen Sound (sounds/results/):'));
		tab_group_data.add(new FlxText(resultsMusicInputText.x, resultsMusicInputText.y - 15, 0, 'Results Screen Music (music/):'));
		...
		tab_group_data.add(new FlxText(barSkinInputText.x, barSkinInputText.y - 15, 0, 'Health & Time Bars Skin:'));
		tab_group_data.add(new FlxText(dsFilterDropDown.x, dsFilterDropDown.y - 15, 0, 'DS Filter Setting:'));
		tab_group_data.add(dsFilterDropDown);
	...
	{
		while (dumUI.members.length > 0)
		{
			dumUI.remove(dumUI.members[0], true);
		}
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if(sender == barSkinInputText) {
				_song.barSkin = barSkinInputText.text;
			}
			...
			else if(sender == resultsBGInputText) {
				_song.resultBG = resultsBGInputText.text;
			}
			else if(sender == resultsSpriteInputText) {
				_song.resultSprite = resultsSpriteInputText.text;
			}
			else if(sender == resultsSoundInputText) {
				_song.resultSound = resultsSoundInputText.text;
			}
			else if(sender == resultsMusicInputText) {
				_song.resultMusic = resultsMusicInputText.text;
			}
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if (FlxG.mouse.wheel != 0)
			{
				...
				if (!mouseQuant)
				...
				else
					{
						...
						if (FlxG.mouse.wheel > 0)
						{
							var yeet:Float = CoolUtil.quantize(beat, snap) - increase;
							FlxG.sound.music.time = Conductor.beatToSeconds(yeet);
						}else{
							var yeet:Float = CoolUtil.quantize(beat, snap) + increase;
							FlxG.sound.music.time = Conductor.beatToSeconds(yeet);
						}
			...
			// What Shadow Mario put here about Arrow Vortex makes NO SENSE!
			// So let me just say: "3 Days until Mario steals your liver! Wahoo!"
			...
			if(!vortex){
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN  )
				{
					...
					{
						var yeet:Float = CoolUtil.quantize(beat, snap) - increase; //(Math.floor((beat+snap) / snap) * snap);
						FlxG.sound.music.time = Conductor.beatToSeconds(yeet);
					}else{
						var yeet:Float = CoolUtil.quantize(beat, snap) + increase; //(Math.floor((beat+snap) / snap) * snap);
						FlxG.sound.music.time = Conductor.beatToSeconds(yeet);
			...
			// Whoever made this look epik is epik themselves...
			...
			if(vortex && !blockInput){
				...
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN  )
				{
					...
					{
						var yeet:Float = CoolUtil.quantize(beat, snap) - increase;
						garbo = Conductor.beatToSeconds(yeet);
					}else{
						var yeet:Float = CoolUtil.quantize(beat, snap) + increase;
						garbo = Conductor.beatToSeconds(yeet);
					}
					FlxTween.tween(FlxG.sound.music, {time:garbo}, 0.1, {ease:FlxEase.circOut});
					if(vocals != null) {
						vocals.pause();
						vocals.time = FlxG.sound.music.time;
					}
					...
					var datime = (garbo - secStart) - (dastrum - secStart); //idk math find out why it doesn't work on any other section other than 0
					if (curSelectedNote != null)
					{
		...
		var playedSound:Array<Bool> = [false, false, false, false]; //Prevents ouchy GF gex sounds
		curRenderedNotes.forEachAlive(function(note:Note) {
			note.alpha = 1;
			if(curSelectedNote != null) {
			...
			if(!justChanged){
				...
				if(note.strumTime <= Conductor.songPosition && FlxG.sound.music.playing){
					if(note.editorBFNote){
						if (!note.hitCausesMiss && !note.noAnimation && !note.isExtraNote) {
							justChanged = true;
							lilBf.animation.play("" + (note.noteData % 4), true);
						}
					}
					else if(!note.editorBFNote){
						if (!note.hitCausesMiss && !note.noAnimation) {
							justChanged = true;
	...
	function resetSection(songBeginning:Bool = false):Void
	{
		...
		lilBf.animation.play("idle");
		lilOpp.animation.play("idle");
		// Basically old junk from changeSection???
	...
	function selectNote(note:Note):Void
	{
		...
		if(noteDataToCheck > -1)
		{
			...
			for (i in _song.notes[curSec].sectionNotes)
			{
				if (i != curSelectedNote && i.length > 2 && i[0] == note.strumTime && i[1] == noteDataToCheck && i[1] == note.absoluteNumber)
				{
					curSelectedNote = i;
					break;
				}
	...
	function deleteNote(note:Note):Void
	{
		...
		if(note.noteData > -1) //Normal Notes
		{
			for (i in _song.notes[curSec].sectionNotes)
			{
				if (i[0] == note.strumTime && i[1] == noteDataToCheck && i[1] == note.absoluteNumber)
				{
	...
	// idk if ShadowMario will EVER figure this out, ever...
	function redo()
	{
