...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	function addAnimationsUI() {
		...
		animationDropDown = new FlxUIDropDownMenu(10, 30, FlxUIDropDownMenu.makeStrIdLabelArray([''], true), function(animation:String) {
			...
			if(character.dialogueAnimations.exists(anim)) {
				...
				var animStuff:DialogueAnimArray = character.dialogueAnimations.get(curSelectedAnim);
				offsetLoopText.text = 'Loop: ' + animStuff.loop_offsets;
				offsetIdleText.text = 'Idle: ' + animStuff.idle_offsets;

				animationInputText.text = animStuff.anim;
				loopInputText.text = animStuff.loop_name;
				idleInputText.text = animStuff.idle_name;
			}
		});
	...
	function reloadCharacter() {
		...
		if(character.jsonFile.animations.length > 0) {
			...
			var animStuff:DialogueAnimArray = character.dialogueAnimations.get(curSelectedAnim);
			ghostLoop.playAnim(animStuff.anim);
			ghostIdle.playAnim(animStuff.anim, true);
			offsetLoopText.text = 'Loop: ' + animStuff.loop_offsets;
			offsetIdleText.text = 'Idle: ' + animStuff.idle_offsets;
		}
			...
			if(UI_mainbox.selected_tab_id == 'Animations' && curSelectedAnim != null && character.dialogueAnimations.exists(curSelectedAnim)) {
				...
				for (i in 0...controlArrayLoop.length) {
					if(controlArrayLoop[i]) {
						if(i % 2 == 1) {
							animStuff.loop_offsets[1] += offsetAdd * negaMult[i];
						} else {
							animStuff.loop_offsets[0] += offsetAdd * negaMult[i];
						}
						moved = true;
					}
				}
				for (i in 0...controlArrayIdle.length) {
					if(controlArrayIdle[i]) {
						if(i % 2 == 1) {
							animStuff.idle_offsets[1] += offsetAdd * negaMult[i];
						} else {
							animStuff.idle_offsets[0] += offsetAdd * negaMult[i];
						}
						moved = true;
					}
				}
				...
				if(moved) {
					offsetLoopText.text = 'Loop: ' + animStuff.loop_offsets;
					offsetIdleText.text = 'Idle: ' + animStuff.idle_offsets;
					ghostLoop.offset.set(animStuff.loop_offsets[0], animStuff.loop_offsets[1]);
					ghostIdle.offset.set(animStuff.idle_offsets[0], animStuff.idle_offsets[1]);
				}
			}
