...
class WeekEditorFreeplayState extends MusicBeatState
{
	...
	override function create() {
		...
		for (i in 0...weekFile.songs.length)
		{
			var songText:CoolScrollText = new CoolScrollText(30, (100 * i) + 400, 0, weekFile.songs[i][0], true, 46);
			songText.targetY = i;
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			weekFile.songs[curSelected][1] = albumInputText.text;
			updateAlbum();
		}
	}

	var albumInputText:FlxUIInputText;
	function addFreeplayUI() {
		var tab_group = new FlxUI(null, UI_box);
		tab_group.name = "Freeplay";

		albumInputText = new FlxUIInputText(10, 35, 100, weekFile.songs[curSelected][1], 8);

		var hideFreeplayCheckbox:FlxUICheckBox = new FlxUICheckBox(10, albumInputText.y + 30, null, null, "Hide Week from Freeplay?", 100);
		...
		tab_group.add(new FlxText(10, albumInputText.y - 18, 0, 'Album Image File Name:'));
	...
	function updateAlbum() {
		albumCover.loadGraphic(Paths.image('albums/' + weekFile.songs[curSelected][1]));
		albumCover.updateHitbox();
	}
	...
	function changeSelection(change:Int = 0) {
		...
		albumInputText.text = weekFile.songs[curSelected][1];
		updateAlbum();
	}
	...
	override function update(elapsed:Float) {
		...
		if(albumInputText.hasFocus) {
			ClientPrefs.toggleVolumeKeys(false);
			if(FlxG.keys.justPressed.ENTER) {
				albumInputText.hasFocus = false;
			}
		} else {
			ClientPrefs.toggleVolumeKeys(true);
			if(FlxG.keys.justPressed.ESCAPE) {
