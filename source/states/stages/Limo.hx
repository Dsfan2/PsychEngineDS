...
class Limo extends BaseStage
{
	...
	public var dodgePole:FlxSprite;

	override function create()
	{
		var skyBG:BGSprite = new BGSprite('stages/limo/limoSunset', -120, -45, 0.1, 0);
		add(skyBG);
		...
		if(songName == 'mild' && (isStoryMode || freeplayCutscenes) && !seenCutscene)
			setStartCallback(startCutscene);
	}
	override function createPost()
	{
		...
		addBehindDad(limo); //REALLY SHADOW MARIO?! How do you screw up THAT badly?!

		dodgePole = new FlxSprite(-500, 220).loadGraphic(Paths.image('stages/limo/dodgeThis'));
		dodgePole.scrollFactor.set(1, 1);
		dodgePole.setGraphicSize(Std.int(dodgePole.width * 1));
		dodgePole.alpha = 0;
		add(dodgePole);
	}
	...
	function poleZoom():Void
	{
		dodgePole.alpha = 1;
		dodgePole.x = -500;
		dodgePole.y = 220;
		dodgePole.velocity.x = 8000;
	}
}