...
class Mall extends BaseStage
{
	...
	override function create()
	{
		...
		if(!ClientPrefs.data.lowQuality) {
			if (songName == 'eggnog')
				upperBoppers = new BGSprite('stages/christmas/upperEggnogBop', -240, -90, 0.33, 0.33, ['Upper Crowd Bob']);
			else
		...
		switch (curPlayer)
		{
			...
			case 2:
				bottomBoppers = new MallCrowd(-300, 140, 'bottomJrBop');
			case 3:
				bottomBoppers = new MallCrowd(-140, 140, 'bottomDokiBop');
		}
		add(bottomBoppers);
	...
	function eggnogEndCutscene()
	{
		...
		if(nextSong == 'winter-horrorland')
		{
			...
			var blackThing:FlxSprite = new FlxSprite(-FlxG.width * FlxG.camera.zoom,
