...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		var blueballedTxt:FlxText = new FlxText(20, 15 + 64, 0, "Game Over'd: " + PlayState.deathCounter, 32);
	...
	function regenMenu():Void {
		for (i in 0...grpMenuStuff.members.length) {
			var obj = grpMenuStuff.members[0];
			obj.kill();
			grpMenuStuff.remove(obj, true);
			obj.destroy();
		}

		for (i in 0...menuItems.length) {
			var item = new Alphabet(90, 320, menuItems[i], true);
			item.setScale(1 * TitleState.alphabetScale);
			item.isMenuItem = true;
			item.targetY = i;
			item.changeX = false;
			grpMenuStuff.add(item);
