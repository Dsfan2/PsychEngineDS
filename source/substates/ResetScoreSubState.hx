...
class ResetScoreSubState extends MusicBeatSubstate
{
	...
	public function new(song:String, difficulty:Int, character:String, week:Int = -1)
	{
		...
		var tooLong:Float = (name.length > 18) ? 0.8 : 1; //Pecking Winter Horrorlandmmmmmmmmmmmmmmmmmm
		var text:Alphabet = new Alphabet(0, 180, "Reset the score of", true);
		text.setScale(1 * TitleState.alphabetScale);
		text.screenCenter(X);
		...
		yesText.setScale(1 * TitleState.alphabetScale);
		yesText.screenCenter(X);
		yesText.x -= 200;
		add(yesText);
		noText = new Alphabet(0, text.y + 150, 'No', true);
		noText.setScale(1 * TitleState.alphabetScale);
